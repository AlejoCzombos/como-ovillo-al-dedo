...
import { redeemPoints } from "@/utils/api.points";
...
export default function CanjearPuntos() {
  ...
  const handleSubmit = async (formData: UpdatePointsForm) => {
    ...
    const toastPromise = toast.loading("Canjeando puntos...");
    const data = await redeemPoints(clientId, amount, token);

    const response = await data.json();
    if (data.status === 200) {
      toast.success(`Puntos canjeados correctamente`, { id: toastPromise });
